...
class RAGChatbot:
    """
    RAG Chatbot:
    1. Retrieval bằng FAISS
    2. Đưa context vào LLM để trả lời
    """
    ...
    def answer(self, question: str, chat_history: list) -> dict:
        ...
        relevant_chunks = self.doc_store.search(
            question,
            top_k=5
        )
        ...
        avg_confidence = round(
            sum(c["confidence"] for c in relevant_chunks) / len(relevant_chunks),
            1
        )

        system_prompt = f"""
Bạn là trợ lý hỏi đáp tài liệu thông minh.

Trả lời câu hỏi DỰA TRÊN tài liệu được cung cấp,
bằng tiếng Việt, rõ ràng và có cấu trúc.

Nếu câu hỏi yêu cầu SO SÁNH nhiều tài liệu,
hãy phân tích từng tài liệu rồi so sánh.

Nếu thông tin không có trong tài liệu,
hãy nói rõ điều đó.
...
"""
    ...
    def compare_documents(self, doc_names: list) -> str:
        ...
        question = (
            "Hãy so sánh chi tiết các tài liệu sau "
            "về: yêu cầu kỹ năng, quy trình, "
            "điều kiện và điểm khác biệt nổi bật: "
            f"{', '.join(doc_names)}"
        )
...
def main():
    ...
    st.caption(
        "RAG + Vector Search + LLaMA 3.3 — "
        "So sánh tài liệu & Xuất báo cáo"
    )
    ...
    with col_left:
        ...
        if uploaded:
            ...
            for file in uploaded:
                ...
                if file.name not in st.session_state.uploaded_files:
                    ...
                    with st.spinner(
                        f"Đang xử lý {file.name}..."
                    ):

                        text = extract_pdf_text(file)

                        n = st.session_state.doc_store.add_document(
                            text,
                            file.name
                        )
        ...
        if st.session_state.uploaded_files:
            ...
            if len(st.session_state.uploaded_files) >= 2:
                ...
                if st.button(
                    "⚖️ So sánh tất cả tài liệu",
                    use_container_width=True
                ):

                    with st.spinner(
                        "Đang phân tích và so sánh..."
                    ):

                        comparison = (
                            st.session_state.chatbot.compare_documents(
                                st.session_state.uploaded_files
                            )
                        )
    ...
    with col_right:
        ...
        with chat_container:
            ...
            if not st.session_state.chat_history:
                st.info(
                    "Upload PDF bên trái rồi đặt câu hỏi!"
                )
                ...
                with st.chat_message(msg["role"]):
                    ...
                    if (
                        ...
                        with cols[1]:
                            st.caption(
                                f"🎯 Độ liên quan: "
                                f"{msg['avg_confidence']}%"
                            )

        question = st.chat_input(
            "Hỏi về tài liệu hoặc so sánh nhiều tài liệu..."
        )
